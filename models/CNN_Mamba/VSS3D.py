...
if __name__ == '__main__':
    ...
    # x = torch.randn(2, 12, 12, 12, 768).cuda()
    x = torch.randn(2, 768, 12, 12, 12).cuda()
    layer = VSSLayer3D(dim=768, depth=1, attn_drop=0.1, mlp_drop=0.1, drop_path=0.1, norm_layer=nn.LayerNorm, use_checkpoint=False, d_state=16, version='v5', expansion_factor=1, scan_type='scan', size=12).cuda()
    out = layer(x)
    print(out.shape)
    # print(out)